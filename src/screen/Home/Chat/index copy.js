                ...
                for (let j = 0; j < result.response[i].otherdata.length; j++) {
                  if (result.response[i].otherdata[j].name !== undefined) {
                    ...
                    if (result.response[i].other_id == mynumber) {
                      const otherId = result.response[i].user_id;
                      console.log('asdfg', otherId);
                      const name = contactsMap[otherId] || result.response[i].user_id;
                      chatname.push(name);
                    } else {
                      const otherId1 = result.response[i].other_id;
                      console.log('asdfg', otherId1);
                      const name = contactsMap[otherId1] || result.response[i].other_id;
                      chatname.push(name);
                    }
                    
                    chatnum.push(chatname);
  ...
  handleChatPress = async (data, index, name) => {     
    ...
    renderItem = ({ item, index }) => {
      ...
      return (
        ...
        <TouchableOpacity style={styles.chatItem} onPress={() => this.handleChatPress(this.state.chats, index, chatName[index])}>
      ...
      render() {
        // const { profileimg, chatName, searchQuery } = this.state;
        const { chats, chatName, profileimg, searchQuery } = this.state;
        // const filteredchats = chatName.filter(chatList =>
        //   chatList.toLowerCase().includes(searchQuery.toLowerCase())
        // );
        return (
