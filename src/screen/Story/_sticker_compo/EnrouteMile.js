...
export class DraggableEvent extends Component {
  ...
  _onHandlerStateChange = event => {
    ...
    //   this.setState({dragBegan : true});
    //   console.log('drag beagan')
    // }
    const {sIndex,sticId,EmitterRoutes} = this.props
    if (event.nativeEvent.oldState === State.ACTIVE) {
      // this.setState({dragBegan : false});
  ...
  onLongPressEvent = () =>{
    ...
    this.props.deleteStickers(!isOnLongPressed,3)
    Vibration.vibrate(300)
  }

 
  render() {
    const {dragBegan,isOnLongPressed} = this.state;
    const {sIndex,SMI,sticId,points,deleteStickers} = this.props;
    /**@PanHandler */
    ...
    return (
      // <View>
      //   {/* {dragBegan
      //    ? <View style={{ padding: 15, backgroundColor: 'red',justifyContent:'center', }}>
            ...
            <TouchableOpacity      
             ...
             >
              ...
              <Animated.Image
                ...
                source={require("../../../assets/stickers/new/enroute_2.png")}
                 resizeMode = {'contain'}
              />
...
export default class EnrouteMile extends Component {
  ...
  render() {
    const {isActive,multiImage,SMI,sIndex,points,
      sticId,EmitterRoutes,deleteStickers} = this.props;
    ...
    return (
      <View style={styles.mainView}>
        ...
        {/* {global.dragBegan[0] != 'true' 
          ?  <View style={{padding:15,backgroundColor:'red'}}>
               <Text style={{color:'#fff'}}>{global.dragBegan[0]+'sd'}drop me</Text>
            </View>
          : null            
         } */}
