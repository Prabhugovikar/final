...
import { API_URL, ProfileImagesPath, socket, ImagePath, VideoPath, AudioPath } from '../../service';
...
class ChatRoom extends Component {
    ...
    async componentDidMount() {
        ...
        const storedTheme = await AsyncStorage.getItem('theme3');
        if (storedTheme) {
            this.setState({
                theme: storedTheme
            });
    ...
    setTheme() {
        ...
        fetch(API_URL + "setthemegroup", requestOptions)
            ...
            .then(result => {
                ...
                AsyncStorage.setItem('theme3', result.result.setTheme);
            })
            .catch(error => console.log('error', error));
    }
    ...
    render() {
        const { Activated, ProfileName, isVisible, modalVisible, names, selectedMessageIds, theme, isOpen } = this.state;
            ...
            {chatMessage.video === '' ? null : (
                ...
                {chatMessage.message === '' ? null : (
                    <View style={{
                        ...
                        <TouchableOpacity
                            onPress={() => this.handlePress(chatMessage._id)}
                            delayLongPress={100}
                        >
                        {/* <Text 
                                style={{
                        ...
                        <View style={styles.footermainview}>
                            ...
                            <Modal
                                ...
                                <View style={styles.bottomView}>
                                    <View style={{ ...styles.modalView, height: 150, width: 250, position: 'absolute', bottom: 25 }}>
                                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                                            <TouchableOpacity onPress={this.openGallery} style={styles.options}>
                                                <View style={styles.options1}>
                                                    <Icon name="image" size={30} color='darkblue' />
                                                </View>
                                            ...
                                            <TouchableOpacity onPress={this.pickAudioFiles} style={styles.options}>
                                                <View style={styles.options1}>
                                                    <Icon name="music" size={30} color='darkblue' />
                                                </View>
