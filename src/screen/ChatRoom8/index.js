...
class ChatRoom extends Component {
    ...
    async componentDidMount() {
        ...
        const storedTheme = await AsyncStorage.getItem('theme9');
        if (storedTheme) {
            this.setState({
                theme: storedTheme
            });
    ...
    setTheme() {
        ...
        fetch(API_URL + "setthemegroup", requestOptions)
            ...
            .then(result => {
                ...
                AsyncStorage.setItem('theme9', result.result.setTheme);
            })
            .catch(error => console.log('error', error));
    }
    ...
    render() {
        const { Activated, ProfileName, isVisible, modalVisible, modalVisible2, search, getmsg, names, contactname, isDisabled, selected, isPlaying, currentVideo, selectedMessageIds, theme } = this.state;
            ...
            {chatMessage.video === '' ? null : (
                ...
                {chatMessage.audio == '' ? null : (
                    ...
                    <TouchableOpacity
                      ...
                      <Text style={{fontWeight:'bold', color:'black', alignSelf:'center', margin:'2%'}}>{chatMessage.audio}</Text>
                      <Text
                            style={{
                                fontSize: Com_font.txt14,
                                color: Com_color.labletxt2,
                    ...
                    <View style={styles.toolmain}>
                        <View style={styles.chatmain}>
                            ...
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('ViewGroup',
                                { id: this.state.group, messages: this.state.chatMessages4, group: this.state.group })}>
                            <Image  source={
                                            this.state.groupPic
