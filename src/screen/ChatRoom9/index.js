...
class ChatRoom extends Component {
    ...
    async componentDidMount() {
        ...
        const storedTheme = await AsyncStorage.getItem('theme10');
        if (storedTheme) {
            this.setState({
                theme: storedTheme
            });
    ...
    setTheme() {
        ...
        fetch(API_URL + "setthemegroup", requestOptions)
            ...
            .then(result => {
                ...
                AsyncStorage.setItem('theme10', result.result.setTheme);
            })
            .catch(error => console.log('error', error));
    }
            ...
            {chatMessage.video === '' ? null : (
                ...
                {chatMessage.image == '' ? null : (
                    <View style={{
                        ...
                        <TouchableOpacity
                            onPress={() => navigation.navigate('Preview', { image: chatMessage.image })}
                            onLongPress={() => this.handlePress(chatMessage._id)}
                            // delayLongPress={100}
                        >
                        <Image
